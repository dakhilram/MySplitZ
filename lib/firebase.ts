...
const configured=Boolean(config.apiKey&&config.projectId&&config.appId);let app:FirebaseApp|undefined;export let db:Firestore|undefined;export let storage:FirebaseStorage|undefined;if(configured){app=getApps().length?getApp():initializeApp(config);db=getFirestore(app);storage=getStorage(app)}
